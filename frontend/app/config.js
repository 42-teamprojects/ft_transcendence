export const config = {
    backend_domain: 'http://10.11.3.3:8000',
    rest_url: 'http://10.11.3.3:8000/api/',
    chat_websocket_url: 'ws://10.11.3.3:8000/ws/chat/',
    match_making_websocket_url: 'ws://10.11.3.3:8000/ws/match-making/',
    match_websocket_url: 'ws://10.11.3.3:8000/ws/match/',
    websocket_url: 'ws://10.11.3.3:8000/ws/',
    finalScore: 3
};
// export const config = {
//     backend_domain: 'http://localhost:8000',
//     rest_url: 'http://localhost:8000/api/',
//     chat_websocket_url: 'ws://localhost:8000/ws/chat/',
//     match_making_websocket_url: 'ws://localhost:8000/ws/match-making/',
//     match_websocket_url: 'ws://localhost:8000/ws/match/',
//     websocket_url: 'ws://localhost:8000/ws/',
//     finalScore: 3
// };

// export const config = {
//     backend_domain: 'http://10.11.3.1:8000',
//     rest_url: 'http://10.11.3.1:8000/api/',
//     chat_websocket_url: 'ws://10.11.3.1:8000/ws/chat/',
//     match_websocket_url: 'ws://10.11.3.1:8000/ws/match/',
//     websocket_url: 'ws://10.11.3.1:8000/ws/',
//     finalScore: 3
// };
