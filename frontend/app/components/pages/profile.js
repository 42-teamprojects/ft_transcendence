...
export default class Profile extends HTMLElement {
  ...
  render() {
    ...
    this.innerHTML = /*html*/ `
        ${
          this.isMine
            ? /*html*/ `<c-upload-avatar-modal></c-upload-avatar-modal>`
            : ""
        }
            ...
            <main>
                ...
                <section class="profile-stats my-8">
                    ...
                    <div class="statistics">
                        <c-statistics-card icon="<img src='/public/assets/icons/streak.svg'/>" number="${
                          this.user.user_stats.current_win_streak
                        }" text="Current Streak"></c-statistics-card>
                        <c-statistics-card icon="<img src='/public/assets/icons/streaks.svg'/>" number="${
                          this.user.user_stats.longest_win_streak
                        }" text="Longest Streak"></c-statistics-card>
                        <c-statistics-card icon="<i class='fa-solid fa-medal text-2xl text-success'></i>" number="${
                          this.user.user_stats.matches_won
                        }" text="Wins"></c-statistics-card>
                        <c-statistics-card icon="<i class='fa-solid fa-times text-2xl text-danger'></i>" number="${
                          this.user.user_stats.matches_lost
                        }" text="Losses"></c-statistics-card>
                        <c-statistics-card icon="<i class='fa-solid fa-trophy text-2xl text-success'></i>" number="${
                          this.user.user_stats.tournaments_won
                        }" text="Tournaments Won"></c-statistics-card>
                        <c-statistics-card icon="<i class='fa-solid fa-times text-2xl text-danger'></i>" number="${
                          this.user.user_stats.tournaments_lost
                        }" text="Tournaments Lost"></c-statistics-card>
                    </div>
                </section>
    ...
    if (blockFriend) {
      ...
      blockFriend.addEventListener("click", () => {
        ...
        this.modal.addEventListener("cancel", () => {
          this.modal.removeEventListener(
            "confirm",
            this.blockFriendFunc.bind(this)
          );
        });
        this.modal.open();
      });
  ...
  getActions() {
    ...
    actions.innerHTML = /*html*/ `
        ${
          this.isMine
            ? /*html*/ `<a is="c-link" href="/dashboard/settings" class="text-secondary btn-link">Edit Profile</a>`
            : ""
        }
        ${
        ...
        ${
          !this.isMine
            ? /*html*/ `<p id="chat-friend" class="cursor-pointer text-secondary btn-link"><i class="fa-regular fa-comment mr-2"></i>Chat</p>`
            : ""
        }
        ${
          !this.isMine && friendState.alreadyFriends(this.user?.id)
  ...
  async handleChatClick() {
    ...
    const chat = chatState.state.chats.find(
      (c) => c.friend.id === this.user.id
    );
  ...
  insertUserStatus() {
    ...
    return /*html*/ `
        ...
        } rounded-full" style="bottom: 10px; right: 30px"></div>
